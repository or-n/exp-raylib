...
func NoiseGenerate(width, height i32, scale f64) {
	buffer := make([]uint8, width*height*4)
	for y := range height {
		for x := range width {
			n := octaveNoise(f64(x)*scale, f64(y)*scale, 8, 0.5)
			gray := uint8(fade(n*0.5+0.5) * 255)
			i := (y*width + x) * 4
			buffer[i+0] = gray
			buffer[i+1] = gray
			buffer[i+2] = gray
	...
	// image := NewImage(buffer, width, height, 1, UncompressedR8g8b8a8)
